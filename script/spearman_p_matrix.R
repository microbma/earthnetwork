spearman_p_matrix <- function(otutable="otutable/rhizosphere.csv"){
        ...
        for(k in 1:960){
                corr.sd.boot  <- (as.matrix(results3[[k]])- corr.mean.boot/960)^2
        }
...
for(i in c(1:14)){
        ...
        write.csv(bray.p$mean,paste("corrP/boot_mean_",otutable.name[i],sep=""))
        write.csv(bray.p$sd,paste("corrP/boot_sd_",otutable.name[i],sep=""))
