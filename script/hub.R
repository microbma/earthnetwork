...
g.hub <- conet
cen.bet.meta <- centr_betw(g.hub,directed = FALSE,normalized = TRUE)[[1]]
names(cen.bet.meta) <- V(g.hub)$name
cen.bet.meta.norm <- (cen.bet.meta)/max(cen.bet.meta)
degree.meta <- degree(g.hub)
names(degree.meta) <- V(g.hub)$name
# Local network
...
for(i in 1:14){
        local.gh <- read_graph(file = paste("graph_1/", 
                                            gh.file[i],sep=""),
                               format = "graphml")
        cen.bet.local <- centr_betw(local.gh,
                                    directed = FALSE,
                                    normalized = TRUE)[[1]]
        names(cen.bet.local) <- V(local.gh)$name
        cen.bet.local.norm <- (cen.bet.local)/max(cen.bet.local)
        df.tem <- data.frame(local = cen.bet.local.norm, 
                             metacommunity = cen.bet.meta.norm[V(local.gh)$name],
                             degree = degree.meta[V(local.gh)$name],
                             env = gsub(".graphml","",gh.file[i]),
                             name = V(local.gh)$name)
        cen.bet.df <- rbind(cen.bet.df, df.tem)
}

cen.bet.deg.df <- cen.bet.df[!is.na(cen.bet.df$degree)&cen.bet.df$degree > 100,]
hub <- rep("Peripheral",dim(cen.bet.deg.df)[1])
hub[cen.bet.deg.df$local < .05 & cen.bet.deg.df$metacommunity < .05] <- "Global hub"
hub[cen.bet.deg.df$local > .05 & cen.bet.deg.df$metacommunity < .05] <- "Metacommunity hub"
hub[cen.bet.deg.df$local < .05 & cen.bet.deg.df$metacommunity > .05] <- "Local hub"

cen.bet.deg.df1 <- cbind(cen.bet.deg.df,hub)
cen.bet.deg.df.r <- cen.bet.deg.df1[cen.bet.deg.df1$name %in% V(g.hub)$name,]
cen.bet.deg.df.r <- cen.bet.deg.df.r[cen.bet.deg.df.r$local != 0 & cen.bet.deg.df.r$metacommunity != 0,]

cen.bet.deg.df.r$env <- factor(cen.bet.deg.df.r$env,labels = env.name1)

ggplot(cen.bet.deg.df.r,aes(y=local,
                      x=metacommunity,
                      color=hub,
                      size=degree))+
        geom_point()+
        facet_wrap(~env,nrow = 7,scales = "fixed")+
        theme_bw()+
        xlab(expression(italic(BC[meta])))+
        ylab(expression(italic(BC[local])))+
        #scale_x_log10()+scale_y_log10()+ 
        #geom_smooth(method="glm")+
        geom_vline(xintercept = .05,linetype=2,color = pal_npg()(4)[4])+
        geom_hline(yintercept = .05,linetype=2,color = pal_npg()(4)[4])+
        scale_size_continuous(range =  c(.3,3),
                              breaks = c(100,200,300,400),
                              labels= c(100,200,300,400),
                              limits =c(100,600),
                              name="Degree")+
        scale_color_manual(values = c(pal_tron(alpha = .4)(5)[c(1,4,3)],
                                      grey(.5,alpha = .5)), 
                           name="Hub type")

ggsave("manuscript/hub_local.tiff",height = 10,width = 7,dpi = 500)

hub.df <- cen.bet.deg.df.r[cen.bet.deg.df.r$hub!="Peripheral",]
...
meta.hub <- hub.table[hub.table$Hub.type == "Global hub",]
meta.hub.tb <- sort(table(meta.hub$Class))
meta.hub.tb <- meta.hub.tb[meta.hub.tb!=0]
pie(meta.hub.tb, cex=.5, 
    col = pal_aaas()(10))

meta.hub1 <- hub.table[hub.table$Hub.type =="Metacommunity hub",]
meta.hub.tb1 <- sort(table(meta.hub1$Class))
meta.hub.tb1 <- meta.hub.tb1[meta.hub.tb1!=0]
pie(meta.hub.tb1, cex=.5, 
    col = pal_aaas()(10))

meta.hub2 <- hub.table[hub.table$Hub.type == "Local hub",]
meta.hub.tb2 <- sort(table(meta.hub2$Class))
meta.hub.tb2 <- meta.hub.tb2[meta.hub.tb2!=0]

require(grid)
require(plotrix)

pdf("pie_hub.pdf",width = 4,height = 8)
par(mfcol=c(3,1),mar=c(0,2,0,2))

pie(meta.hub.tb[21:40], cex=.8, border = NA,init.angle = 125,
    col = qualitative_hcl(15,alpha = .6,palette = "dark3"))
draw.circle(0,0,radius = .6, border = "white", col = "white")
text(0,0,labels = "Global hubs",font = 2,cex = 1.1)

pie(meta.hub.tb1[1:11], cex=.8, border = NA,init.angle = 135,
    col = qualitative_hcl(15,alpha = .6,palette = "dark3"))
draw.circle(0,0,radius = .6, border = "white", col = "white")
text(0,0,
     labels = "Metacommunity hubs",
     font = 2,
     cex = 1.2)

pie(meta.hub.tb2[22:41], cex=.8, border = NA,init.angle = 125,
    col = qualitative_hcl(15,alpha = .6,palette = "dark3"))
draw.circle(0,0,radius = .6, border = "white", col = "white")
text(0,0,labels = "Local hubs",font = 2,cex = 1.2)
dev.off()
...
hub1.node.df <- cen.bet.deg.df.r[grep(pattern = "Metacommunity hub", hub),]
V(g.hub)$hub[V(g.hub)$name %in% as.character(hub1.node.df$name)] <- "Metacommunity hub"
hub2.node.df <- cen.bet.deg.df.r[grep(pattern = "Local hub", hub),]
V(g.hub)$hub[V(g.hub)$name %in% as.character(hub2.node.df$name)] <- "Local hub"
hub3.node.df <- cen.bet.deg.df.r[grep(pattern = "Global hub",hub),]
...
local <- data.frame(V(g.hub)$s1,
                    ...
                    V(g.hub)$s14)
for(i in 1:14){
        local.hub <- unique(hub.df[hub.df$hub == "Local hub" & hub.df$env == env.name1[i], 5])
        meta.hub <- unique(hub.df[hub.df$hub == "Metacommunity hub" & hub.df$env == env.name1[i], 5])
        global.hub <- unique(hub.df[hub.df$hub == "Global hub" & hub.df$env == env.name1[i],5])
        ...
        E(g.hub)$color <- grey(.8,alpha = .01)
        E(g.hub)$color[g1.edgelist[,1] %in%  local.hub|g1.edgelist[,2] %in%  local.hub] <- pal_tron(alpha=.4)(5)[c(4)]
        E(g.hub)$color[g1.edgelist[,1] %in%  meta.hub|g1.edgelist[,2] %in%  meta.hub] <- pal_tron(alpha=.4)(5)[c(3)]
        E(g.hub)$color[g1.edgelist[,1] %in%  global.hub|g1.edgelist[,2] %in%  global.hub] <- pal_tron(alpha=.4)(5)[c(1)]
        ...
        plot(g.hub,
             ...
             vertex.label = NA,
             vertex.size = log(degree(g.hub)+1),
             vertex.color =c(grey(.7,alpha = .3),pal_tron(alpha=.3)(5)[c(2,4,3,1)])[v.shape+1],
             vertex.frame.color=NA,
             edge.width =.5,
             edge.curved = TRUE)
        dev.off()
...
ggplot(cen.bet.df, aes(x = env2, y = metacommunity))+
    ...
    scale_color_discrete_qualitative(alpha = .6)+
    theme_bw()+
    ylab(expression(italic(BC[meta])))+
    xlab("Environmental type")+
    theme(panel.grid = element_blank(),
